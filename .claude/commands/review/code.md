...
**Input**: `/review:code <ファイルパスまたはディレクトリパス>` の形式で対象を指定。複数のパスをスペース区切りで指定可能。省略時はユーザーに確認する。
...
- **具体的であること**: すべての指摘にファイルパスと行番号を含める
- **実行可能であること**: 各指摘に具体的な修正方法を提示する
- **バランス**: 問題点だけでなく、良い実装も認める（GOOD）
- **優先度の明確化**: CRITICAL > WARNING > SUGGESTION の順で重要度を示す
- **過度な指摘を避ける**: スタイルの好みレベルの指摘は控え、実質的な問題に集中する
