...
- [ ] `src/hooks/useStaff.test.ts` に `reorderStaff` のテストを追加する（Red）
...
- [ ] `src/hooks/useStaff.ts` に `reorderStaff(fromIndex: number, toIndex: number)` を実装する（Green）
...
- [ ] `src/hooks/useDragSort.test.ts` を作成しテストを書く（Red）
  - 「初期状態では `draggingIndex` と `dragOverIndex` が null である」
  - 「`getDragHandleProps(0)` の `onPointerDown` を呼ぶと `draggingIndex` が 0 になる（マウス操作は即座に開始）」
...
- [ ] `src/hooks/useDragSort.ts` を作成し実装する（Green）
  - Props: `onReorder: (fromIndex: number, toIndex: number) => void`
...
- [ ] `src/pages/StaffPage.test.tsx` にD&D関連テストを追加する（Red）
...
- [ ] `src/pages/StaffPage.tsx` を更新し D&D を実装する（Green）
...
- [ ] `openspec/specs/staff-management/spec.md` にスタッフ並び替え要件を追記する
...
- [ ] `useDragSort` をリファクタリングする（Refactor）
  - イベントリスナーの登録・解除が適切に行われているか確認する
  - `useCallback`/`useRef` を適切に使用してパフォーマンスを確保する

- [ ] 全テストが通ることを確認する（`bun run test`）

- [ ] ビルドエラーがないことを確認する（`bun run build`）
