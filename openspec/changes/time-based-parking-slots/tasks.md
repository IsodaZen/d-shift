...
- [ ] `src/utils/shiftUtils.test.ts` に時間帯ベース共有の新規テストを追加する（Red）
...
- [ ] `src/utils/shiftUtils.ts` の `assignParking` 関数に `timeSlot: TimeSlot` 引数を追加する（Green）
...
- [ ] `src/hooks/useAssignments.ts` の `addAssignment` 内で `assignParking` 呼び出し時に `timeSlot` を渡す（Green）

- [ ] `src/utils/autoShiftGenerator.ts` の `generateAutoShift` 内で `assignParking` 呼び出し時に適切な `timeSlot` を渡す（Green）
