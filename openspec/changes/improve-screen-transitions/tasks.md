...
- [x] `react-router-dom`（v7系）を `dependencies` に追加する
...
- [x] `vite.config.ts` に `spaFallback()` カスタムプラグインを追加する
...
- [x] `src/components/AppHeader.test.tsx` を作成する（Red）
...
- [x] `src/components/AppHeader.tsx` を実装する（Green）
...
- [x] `src/App.test.tsx` を新規作成する（Red）
...
- [x] `src/pages/SettingsPage.test.tsx` にURLタブ連動テストを追加する（Red）
...
- [x] `src/pages/SettingsPage.test.tsx` にフロー誘導 CTA テストを追加する（Red）
...
- [x] `src/pages/SettingsPage.tsx` を更新する（Green）
...
- [x] `src/pages/StaffPage.test.tsx` を新規作成する（Red）
...
- [x] `src/pages/StaffPage.tsx` を更新する（Green）
...
- [x] `src/pages/SettingsPage.test.tsx` の既存テストを `MemoryRouter` でラップする
  - `useParams` / `useNavigate` 導入後にルーターコンテキストが必要になるため
  - 既存のすべてのテストが引き続きパスすることを確認する（`bun run test`）

- [x] `src/pages/ShiftPage.test.tsx` の既存テストを `MemoryRouter` でラップする（必要な場合のみ）
  - ShiftPage に `useNavigate` 等を追加した場合のみ対応する（今回は変更なし）
...
- [x] ビルド後に `dist/404.html` が生成されていることを確認する（`ls dist/404.html`）
