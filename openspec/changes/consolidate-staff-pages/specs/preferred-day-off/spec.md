...
システムは、スタッフ単位・日付単位で希望休を登録できなければならない（SHALL）。登録された希望休はLocalStorageに永続化される。希望休管理UIは StaffPage（`/`）内の希望休管理ビューに配置する（各スタッフアイテムの「希望休」ボタンから遷移する）。

> **変更点**: 設置場所が SettingsPage（`/settings/dayoff`）から StaffPage（`/`）の希望休管理ビューに変更された。希望休管理ビューは各スタッフアイテムの「希望休」ボタンをクリックして表示する。スタッフ選択ドロップダウンは不要（対象スタッフはボタンを押したスタッフが固定される）。UI仕様・ロジックは変更しない。

#### Scenario: 希望休を登録できる
- **WHEN** ユーザーが特定のスタッフの希望休管理ビューで日付を選択して希望休を登録する
- **THEN** その希望休がLocalStorageに保存される

#### Scenario: 同一スタッフ・同一日付の希望休は重複登録できない（フォールバックUI）
- **GIVEN** `isShiftPeriodSaved === false`（フォールバックUIが表示されている）
- **WHEN** ユーザーがすでに登録済みの日付の希望休を再度登録しようとする
- **THEN** 重複エラーが表示され、登録は行われない

#### Scenario: カレンダーUIでは同一日付が既存の選択済み状態として表示されるため重複は発生しない
- **GIVEN** `isShiftPeriodSaved === true`（カレンダーUIが表示されている）
- **AND** スタッフAの date1 が登録済みである
- **WHEN** ユーザーがスタッフAの「希望休」ボタンから希望休管理ビューを開く
- **THEN** date1 は初期状態から選択済み（ハイライト）で表示される
- **AND** 重複エラーは発生しない（保存操作は `syncDayOffs` による上書き同期であるため）

---

### Requirement: カレンダーUIで希望休を編集・保存（同期）できる

システムは、対象スタッフを固定した状態でカレンダーUIから複数の日付をトグル選択し、「保存」ボタンで希望休をLocalStorageに同期できなければならない（SHALL）。カレンダーUIは StaffPage（`/`）内の希望休管理ビューに表示する。

UIフロー（シフト期間設定済みの場合）:
1. スタッフ一覧でスタッフAの「希望休」ボタンをクリックする（スタッフ選択ドロップダウンは不要）
2. スタッフAの希望休管理ビューが表示され、シフト期間の開始月からカレンダーが表示される
...
#### Scenario: 希望休管理ビューを開くと登録済み日付が選択済み状態でカレンダーに表示される

- **GIVEN** シフト期間が設定されている
- **AND** スタッフAの希望休として date1、date2 が登録済みである
- **WHEN** ユーザーがスタッフAの「希望休」ボタンをクリックして希望休管理ビューを開く
- **THEN** 「スタッフA の希望休」という見出しとともに、シフト期間の開始月からカレンダーが表示される
- **AND** date1、date2 は選択済み（ハイライト）状態で表示される
- **AND** シフト期間外の日付はグレーアウトして選択不可で表示される

#### Scenario: カレンダー上の日付をタップして選択・解除できる

- **GIVEN** 希望休管理ビューのカレンダーが表示されている
...
#### Scenario: 保存後もカレンダーの選択状態は維持される

- **GIVEN** スタッフAの希望休管理ビューで date2 が選択された状態で「保存」ボタンを押した直後である
- **WHEN** 保存が完了する
- **THEN** カレンダー上の date2 の選択状態は維持される（保存後にリセットしない）
...
- **AND** 希望休管理ビューを開いた直後（date1 が初期選択済み状態）から何も変更していない
- **WHEN** ユーザーが「保存」ボタンを押す
...
システムは、`isShiftPeriodSaved` が `false`（ユーザーが明示的にシフト期間を保存していない）の場合、従来の単一日付入力（`<input type="date">`）による1件登録UIを StaffPage の希望休管理ビューに提供しなければならない（SHALL）。

#### Scenario: シフト期間が未保存（isShiftPeriodSaved = false）の場合は日付入力フィールドが表示される

- **GIVEN** ユーザーがシフト期間を明示的に保存したことがない（`isShiftPeriodSaved === false`）
- **WHEN** ユーザーがいずれかのスタッフの「希望休」ボタンをクリックして希望休管理ビューを開く
- **THEN** カレンダーUIではなく、日付入力フィールド（`<input type="date">`）が表示される
- **AND** 日付を選択して1件ずつ登録できる（既存仕様の挙動を維持する）
...
### Requirement: 希望休一覧をスタッフ別の件数サマリーで表示する（グローバルタブ）
**Reason**: グローバルな「希望休」タブを廃止したため、タブ上のスタッフ別サマリーも廃止する。各スタッフの希望休管理は StaffPage の各スタッフアイテムにある「希望休」ボタンから行う。

### Requirement: セレクトボックスでスタッフを切り替えると選択状態がリセットされる
**Reason**: per-staff ビューではスタッフ選択ドロップダウンが存在しないため、このシナリオは廃止する。
