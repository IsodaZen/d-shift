# preferred-day-off Delta Spec（希望休一括登録・カレンダーUI）

この仕様は `openspec/specs/preferred-day-off/spec.md` の拡張仕様である。

**変更・廃止する既存要件:**
- `Requirement: 希望休を削除できる`（個別削除UI） → 廃止。削除操作はカレンダーUIの選択解除＋保存で代替する
- `Requirement: 希望休の一覧を確認できる`（月単位リスト） → 本仕様の「スタッフ別サマリー表示」に変更

**既存のフック関数の扱い:**
- `addDayOff`（単数形）: フォールバックUIで引き続き使用するため維持する
- `deleteDayOff`: UIから直接呼ばれなくなるが、フック関数自体は削除しない（`syncDayOffs` が内部的に同等の操作を行う）
