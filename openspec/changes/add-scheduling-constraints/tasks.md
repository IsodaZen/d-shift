...
- [ ] `src/utils/autoShiftGenerator.test.ts` に制約1（全時間帯出勤の原則）のテストを追加する（Red）
...
- [ ] `src/utils/autoShiftGenerator.test.ts` に制約2（必要人数超過禁止）のテストを追加する（Red）
...
- [ ] `src/utils/autoShiftGenerator.ts` のアルゴリズムを「日」単位処理に変更する（Green）
